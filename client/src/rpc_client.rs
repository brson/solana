...
/// [`Processed`] commitment level. These exceptions are noted in the
...
impl RpcClient {
    ...
    ///
    /// The encodings are returned in [`UiTransactionEncoding::Json`][uite] format.
    /// To return transactions in other encodings, use
    ...
    ///
    /// The range is inclusive, with results including the block
    /// for both `start_slot` and `end_slot`.
    ///
    /// If `end_slot` is not provided, then the end slot is
    /// for the latest confirmed block.
    ///
    ...
    /// # Errors
    ///
    /// This method returns an error if the range is greater than 500,000
    /// slots.
    ///
    /// # RPC Reference
    ///
    /// This method corresponds directly to the [`getBlocks`] RPC
    /// method, unless the remote node version is less than 1.7,
    /// in which case it maps to the [`getConfirmedBlocks`] RPC method.
    ///
    ...
    /// This method may not return blocks for the full range of slots if some
    /// slots do not have corresponding blocks. To simply get a specific number
    /// of sequential blocks, use the [`get_blocks_with_limit_and_commitment`] method.
    ///
    ...
    /// slots.
    ///
    /// This method returns an error if the given commitment level
    /// is below [`Confirmed`].
    ///
    ...
    /// # Errors
    ///
    /// This method returns an error if the limit is greater than 500,000
    /// slots.
    ///
    /// # RPC Reference
    ///
    /// This method corresponds directly to the [`getBlocksWithLimit`]
    /// RPC method, unless the remote node version is less than 1.7,
    /// in which case it maps to the [`getConfirmedBlocksWithLimit`] RPC method.
    ///
    ...
    /// RPC method, unless the remote node version is less than 1.7,
    /// in which case it maps to the [`getSignaturesForAddress2`] RPC method.
    ///
    ...
    /// RPC method, unless the remote node version is less than 1.7,
    /// in which case it maps to the [`getConfirmedTransaction`] RPC method.
    ///
    ...
    ///
    /// This value may increase over time if the node is configured to
    /// purge older ledger data.
    ///
    /// # RPC Reference
    ///
    ...
    ///
    /// Once this function returns successfully, the given transaction
    /// is guaranteed to be processed with the configured commitment level.
    ///
    /// After sending the transaction, this method polls in a loop for
    /// the status of the transaction until it has ben confirmed.
    ///
    /// # Errors
    ///
    /// If the transaction is not signed then an error with kind [`RpcError`] is
    ...
    ///
    /// let bogus_pubkey = Pubkey::from_str("not-a-real-key").unwrap();
    /// let maybe_account = rpc_clent.get_account(&bogus_pubkey);
    /// assert!(maybe_account.is_err());
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    ///
    /// This is equivalent to calling [`get_account`] and then accessing
    /// the [`data`] field of the returned [`Account`].
    ///
